...
import {
  ...
  type PayPhoneAdminConfiguration,
} from "@/lib/api"
...
const EMPTY_PAYPHONE_CONFIG: PayPhoneAdminConfiguration = {
...
const getMaskedToken = (token: string) => {
  ...
  if (!normalized) return "No configurado"
...
export default function BankAccountsManagement() {
  ...
  const [payPhoneConfig, setPayPhoneConfig] = useState<PayPhoneAdminConfiguration>(EMPTY_PAYPHONE_CONFIG)
  const [isPayPhoneLoading, setIsPayPhoneLoading] = useState(true)
  const [isPayPhoneSaving, setIsPayPhoneSaving] = useState(false)
  ...
  const loadPayPhoneConfiguration = async () => {
    const token = localStorage.getItem("auth_token")
    if (!token) {
      setIsPayPhoneLoading(false)
      return
    }

    try {
      setIsPayPhoneLoading(true)
      const configuration = await payphoneApi.getAdminConfiguration(token)
      setPayPhoneConfig(configuration)
    } catch (error) {
      const description = error instanceof Error ? error.message : "No se pudo cargar la configuración de PayPhone."
      toast({
        title: "Error",
        description,
        variant: "destructive",
  ...
  const handlePayPhoneFieldChange = (field: keyof PayPhoneAdminConfiguration, value: string | boolean) => {
    setPayPhoneConfig((prev) => ({
      ...prev,
      [field]: value,
    }))
  }
  ...
  const handleSavePayPhoneConfiguration = async () => {
    const token = localStorage.getItem("auth_token")
    if (!token) return

    setIsPayPhoneSaving(true)

    try {
      const updatedConfiguration = await payphoneApi.updateAdminConfiguration(
        {
          ...payPhoneConfig,
          PAYPHONE_TOKEN: payPhoneConfig.PAYPHONE_TOKEN.trim(),
          PAYPHONE_STORE_ID: payPhoneConfig.PAYPHONE_STORE_ID.trim(),
          PAYPHONE_RESPONSE_URL: payPhoneConfig.PAYPHONE_RESPONSE_URL.trim(),
          PAYPHONE_CANCELLATION_URL: payPhoneConfig.PAYPHONE_CANCELLATION_URL.trim(),
        },
        token,
      )

      setPayPhoneConfig(updatedConfiguration)
      toast({ title: "Configuración de PayPhone guardada" })
    } catch (error) {
      const description = error instanceof Error ? error.message : "No se pudo guardar la configuración de PayPhone."
      toast({ title: "Error", description, variant: "destructive" })
    } finally {
      setIsPayPhoneSaving(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
      <Card className="bg-white border-gray-200">
        <CardHeader>
          <CardTitle>Configuración de PayPhone</CardTitle>
          <CardDescription>
            Solo administradores pueden editar estas credenciales y URLs desde <span className="font-medium">/admin/bancos</span>.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isPayPhoneLoading ? (
            <div className="flex items-center gap-2 py-10 text-sm text-gray-500">
              ...
              Cargando configuración de PayPhone...
            </div>
          ) : (
            <div className="space-y-5">
              <div className="grid gap-4 md:grid-cols-2">
                <div className="grid gap-2">
                  <Label htmlFor="payphone-enabled">PayPhone habilitado</Label>
                  <div className="flex items-center gap-3 rounded-md border border-gray-200 px-3 py-2">
                    <Switch
                      id="payphone-enabled"
                      checked={payPhoneConfig.PAYPHONE_ENABLED}
                      onCheckedChange={(checked) => handlePayPhoneFieldChange("PAYPHONE_ENABLED", checked)}
                    />
                    <span className="text-sm text-gray-600">
                      {payPhoneConfig.PAYPHONE_ENABLED ? "Activo" : "Inactivo"}
                    </span>
                  </div>
                </div>
                ...
                <div className="grid gap-2">
                  <Label>Resumen</Label>
                  <div className="rounded-md border border-dashed border-gray-200 p-3 text-sm text-gray-600 space-y-1">
                    <div><span className="font-medium">Token:</span> {getMaskedToken(payPhoneConfig.PAYPHONE_TOKEN)}</div>
                    <div><span className="font-medium">Store ID:</span> {payPhoneConfig.PAYPHONE_STORE_ID || "No configurado"}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid gap-4">
                <div className="grid gap-2">
                  <Label htmlFor="payphone-token">PAYPHONE_TOKEN</Label>
                  <Input
                    id="payphone-token"
                    type="text"
                    value={payPhoneConfig.PAYPHONE_TOKEN}
                    onChange={(event) => handlePayPhoneFieldChange("PAYPHONE_TOKEN", event.target.value)}
                    placeholder="Token de PayPhone"
                  />
                </div>

                <div className="grid gap-2">
                  <Label htmlFor="payphone-store-id">PAYPHONE_STORE_ID</Label>
                  <Input
                    id="payphone-store-id"
                    type="text"
                    value={payPhoneConfig.PAYPHONE_STORE_ID}
                    onChange={(event) => handlePayPhoneFieldChange("PAYPHONE_STORE_ID", event.target.value)}
                    placeholder="UUID del comercio"
                  />
                </div>

                <div className="grid gap-2">
                  <Label htmlFor="payphone-response-url">PAYPHONE_RESPONSE_URL</Label>
                  <Input
                    id="payphone-response-url"
                    type="url"
                    value={payPhoneConfig.PAYPHONE_RESPONSE_URL}
                    onChange={(event) => handlePayPhoneFieldChange("PAYPHONE_RESPONSE_URL", event.target.value)}
                    placeholder="https://profesionales.ec/payphone/priority/resultado"
                  />
                </div>

                <div className="grid gap-2">
                  <Label htmlFor="payphone-cancellation-url">PAYPHONE_CANCELLATION_URL</Label>
                  <Input
                    id="payphone-cancellation-url"
                    type="url"
                    value={payPhoneConfig.PAYPHONE_CANCELLATION_URL}
                    onChange={(event) => handlePayPhoneFieldChange("PAYPHONE_CANCELLATION_URL", event.target.value)}
                    placeholder="https://profesionales.ec/crear-perfil?plan=priority&payment=payphone"
                  />
                </div>
              </div>

              <div className="flex justify-end">
                <Button onClick={handleSavePayPhoneConfiguration} disabled={isPayPhoneSaving} className="bg-emerald-600 hover:bg-emerald-700">
                  {isPayPhoneSaving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}
